...
```shell-session
LordGreb@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
LordGreb@htb[/htb]$ cat target.nmap
...
```shell-session
LordGreb@htb[/htb]$ cat target.gnmap
...
```shell-session
LordGreb@htb[/htb]$ cat target.xml
...
```shell-session
LordGreb@htb[/htb]$ xsltproc target.xml -o target.html
```
